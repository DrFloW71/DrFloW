// ==UserScript==
// @name         WEDA CIM10 Batch - module autonome
// @namespace    http://tampermonkey.net/
// @version      2.20.12
// @description  Module batch pour lancer le codage CIM-10 Heidi/WEDA sur plusieurs patients à la suite. V2.20.12 : garde anti double lancement CIM-10 compatible avec CIM10 5.0.10.
...
(function () {
    ...
    const VERSION_BATCH = '2.20.12';
    const HOST_WEDA = 'secure.weda.fr';
    const LOG_PREFIX = '[AUTO-ATCD-BATCH]';
    const WORKER_HASH_PREFIX = 'AUTO_ATCD_BATCH_WORKER=';

    const BATCH_KEY_QUEUE = 'auto_atcd_cim10_batch_queue_v1';
    const BATCH_KEY_STATE = 'auto_atcd_cim10_batch_state_v1';
    const BATCH_KEY_LOG = 'auto_atcd_cim10_batch_log_v1';
    const BATCH_KEY_CURRENT = 'auto_atcd_cim10_batch_current_v1';
    const BATCH_KEY_COMMAND = 'auto_atcd_cim10_batch_command_v1';
    const BATCH_KEY_WORKER_HEARTBEAT = 'auto_atcd_cim10_batch_worker_heartbeat_v1';
    const BATCH_KEY_RESULT = 'auto_atcd_cim10_batch_result_v1';
    const BATCH_KEY_LOCK = 'auto_atcd_cim10_batch_lock_v1';
    const BATCH_KEY_CHAIN_LOCK = 'auto_atcd_cim10_batch_chain_lock_v1';
    ...
    const CIM10_IMPORT_WAKE_KEY = 'auto_atcd_cim10_import_wake_v1';
    const SESSION_BATCH_WORKER_INFO = 'auto_atcd_cim10_batch_worker_info_v1';
    const SESSION_BATCH_ANTECEDENTS_RELOAD_RETRY = 'auto_atcd_cim10_batch_antecedents_reload_retry_v1';
    const SESSION_BATCH_CIM10_LAUNCH = 'auto_atcd_cim10_batch_cim10_launch_v1';

    const PAGE_LOAD_TIMEOUT_MS = 30000;
    const ANTECEDENTS_NAV_TIMEOUT_MS = 45000;
    const CIM10_BUTTON_TIMEOUT_MS = 45000;
    ...
    const PANEL_ID = 'auto-atcd-batch-panel';
    const LOG_PANEL_ID = 'auto-atcd-batch-log-panel';
    ...
    const ACTIVE_STATUSES = new Set([
        ...
        'waiting_cim10_button',
        'clicking_cim10_button',
        'waiting_cim10_done',
        'closing_worker',
        'next_patient'
    ]);
    ...
    const CONTROLLER_ID = getOrCreateSessionValue('auto_atcd_batch_controller_id_v1', () => {
    ...
    function installPanel() {
        ...
        panel.innerHTML = [
            ...
            '<strong style="font-size:13px;">CIM10 batch</strong>',
    ...
    function renderBatchLogPanelContent(panel) {
        ...
        panel.innerHTML =
            '<div style="position:sticky;top:0;background:#102f4e;color:#fff;padding:12px 14px;display:flex;gap:8px;align-items:center;z-index:1">' +
                '<strong style="flex:1">Journal batch CIM10</strong>' +
    ...
    async function startBatch() {
        ...
        if (pendingCount > 10 && !confirm(`Lancer le codage CIM-10 pour ${pendingCount} patients ?`)) {
    ...
    function focusWorkerTab(worker, patient, reason = '') {
        ...
        if (focused) {
            addLog('warn', 'Worker WEDA remis au premier plan pour relancer l’import.', patient, {
                phase: 'worker_focus_after_stale_heartbeat',
                reason,
    ...
    async function waitForWorkerOutcome(patient, batchId, worker, launchedAt) {
        ...
        while (nowMs() - launchedAt < PATIENT_TIMEOUT_MS) {
            ...
            if (directBridgeOutcome) {
                ...
                addLog('success', 'Resultat CIM-10 recu par pont localStorage.', patient, {
            ...
            if (tabClosed) {
                if (clicked) {
                    ...
                    addLog('error', 'Onglet worker ferme avant statut terminal CIM-10 structure.', patient, {
                    ...
                    return {
                        ...
                        patientId: patient.id,
                        status: 'error',
                        message: 'Onglet worker ferme avant statut terminal CIM-10 structure.',
                        ts: nowMs(),
                        report: { reason: 'worker_closed_before_terminal_cim10_status', lastHeartbeat }
                    };
                }

                addLog('error', 'Onglet worker ferme avant lancement CIM-10.', patient, {
                    phase: 'worker_tab_closed',
                    clicked,
                    elapsedMs: nowMs() - launchedAt,
                    lastHeartbeat
                ...
                return {
                    ...
                    patientId: patient.id,
                    status: 'error',
                    message: 'Worker ferme avant lancement CIM-10.',
                    ts: nowMs()
                };
            }
            ...
            if (lastHeartbeat && nowMs() - Number(lastHeartbeat.ts || 0) > HEARTBEAT_STALE_MS) {
                if (clicked) {
                    ...
                    if (!staleAfterClickLogged) {
                        ...
                        addLog('warn', 'Heartbeat worker en retard apres lancement CIM-10 : attente du statut terminal.', patient, {
                ...
                if (!clicked) {
                    const earlyStageBudget = PAGE_LOAD_TIMEOUT_MS + ANTECEDENTS_NAV_TIMEOUT_MS + CIM10_BUTTON_TIMEOUT_MS;
                    if (nowMs() - launchedAt > earlyStageBudget) {
                        addLog('error', 'Timeout : worker silencieux avant lancement CIM-10.', patient, {
                        ...
                        return {
                            ...
                            patientId: patient.id,
                            status: 'timeout',
                            message: 'Timeout : worker silencieux avant lancement CIM-10.',
                            ts: nowMs(),
    ...
    function isAfterCim10ClickStatus(status) {
        return status === 'clicking_cim10_button' || status === 'waiting_cim10_done';
    }
    ...
    function parseWorkerInfoFromHash() {
        ...
        const batchMatch = hash.match(/(?:^|&)AUTO_ATCD_BATCH_WORKER=([^&]+)/);
    ...
    function publishWorkerResult(status, message, report) {
        ...
        clearPersistedCim10Launch(info);
    ...
    async function runWorker() {
        ...
        if (window.__AUTO_ATCD_BATCH_WORKER_RUNNING__) return;
        window.__AUTO_ATCD_BATCH_WORKER_RUNNING__ = true;
        ...
        try {
            ...
            if (!currentAntecedentPatDk || !samePatDk(info.patientId, currentAntecedentPatDk)) {
                publishWorkerResult('error', 'Sécurité patient : lancement CIM-10 bloqué, onglet Antécédents sur un autre patient.', {
                    stage: 'patient_identity_guard_before_cim10',
            ...
            const baselineReports = readCim10Reports();
            const persistedLaunch = getPersistedCim10Launch(info);
            if (persistedLaunch) {
                setWorkerStatus('waiting_cim10_done');
                runtime.workerClickedAt = Number(persistedLaunch.ts || 0) || nowMs();
                publishWorkerHeartbeat();
                addLog('info', 'Codage CIM-10 deja lance pour ce patient : attente du resultat, sans nouveau clic.', patientForLog, {
                    phase: 'worker_cim10_persisted_launch_wait',
                ...
                const outcome = await waitForCim10Done(runtime.workerClickedAt, baselineReports, remainingTimeout, {
                    batchId: info.batchId,
                    patientId: info.patientId
                });
            ...
            const activeCim10Job = getActiveCim10JobForPatient(getExpectedWorkerPatientId(patientDescriptor, info.patientId), {
                batchId: info.batchId,
                patientId: info.patientId
            });
            if (activeCim10Job) {
                setWorkerStatus('waiting_cim10_done');
                runtime.workerClickedAt = getCim10JobTimestamp(activeCim10Job);
                markPersistedCim10Launch(info, {
                    ts: runtime.workerClickedAt,
                    method: 'existing_cim10_job',
                    observedStarted: true
                }, patientForLog, 'existing_job_detected');
                publishWorkerHeartbeat();
                addLog('info', 'Codage CIM-10 deja en cours pour ce patient : attente du resultat existant.', patientForLog, {
                    phase: 'worker_cim10_existing_job_wait',
                    job: summarizeReport(activeCim10Job),
                    jobPatientIds: getCim10JobPatientIds(activeCim10Job),
                    elapsedMs: nowMs() - workerStartedAt,
                    skipDiagnostic: true
                });
            ...
            const button = await waitFor(() => findExistingCim10Button(), CIM10_BUTTON_TIMEOUT_MS, 500);
            let launchAttempt = null;

            if (button) {
                addLog('success', 'Bouton CIM-10 trouve.', patientForLog, {
                    phase: 'worker_cim10_button_found',
                    id: button.id || '',
                    text: getElementText(button)
                });

                setWorkerStatus('clicking_cim10_button');
                launchAttempt = await triggerCim10Launch(button, baselineReports, patientForLog);
            } else if (isStrongAntecedentsPage() && getCim10ApiFunction('AUTO_ATCD_CIM10_START')) {
                addLog('warn', 'Bouton CIM-10 introuvable, tentative via API exposee du script antecedents.', patientForLog, {
                    phase: 'worker_cim10_button_missing_api',
                    url: window.location.href
                });
                setWorkerStatus('clicking_cim10_button');
                launchAttempt = await triggerCim10Launch(null, baselineReports, patientForLog);
            } else if (isStrongAntecedentsPage()) {
                addLog('warn', 'Bouton CIM-10 introuvable sur la page antecedents.', patientForLog, {
                    phase: 'worker_cim10_button_missing',
                    url: window.location.href
                });
                if (reloadAntecedentsPageForCim10Retry(patientForLog, 'button_missing')) return;
            }

            if (!launchAttempt) {
                publishWorkerResult('timeout', 'Timeout : bouton CIM-10 existant introuvable.', {
                    stage: 'waiting_cim10_button',
                    url: window.location.href
                });
                return closeCurrentWorkerSoon();
            }

            if (!launchAttempt.observedStarted) {
                if (reloadAntecedentsPageForCim10Retry(patientForLog, 'launch_unconfirmed')) return;
            }

            runtime.workerClickedAt = launchAttempt.ts;
            if (launchAttempt.observedStarted) {
                markPersistedCim10Launch(info, launchAttempt, patientForLog, 'observed_started');
            }
            publishWorkerHeartbeat();
            addLog('info', `Codage CIM-10 lance pour ${patientDescriptor.patDk ? 'PatDk=' + patientDescriptor.patDk : 'patient=' + patientDescriptor.name + ' (' + info.patientId + ')'}.`, patientForLog, {
                phase: 'worker_cim10_started',
                method: launchAttempt.method,
                observedStarted: launchAttempt.observedStarted,
                elapsedMs: nowMs() - workerStartedAt
            });
        ...
        setTimeout(() => {
            try {
                ...
                if (openerWindow && !openerWindow.closed && typeof openerWindow.focus === 'function') {
                    openerWindow.focus();
                    addLog('info', 'Retour onglet controleur avant fermeture worker.', { id: info.patientId }, {
    ...
    function classifyLocalStorageBridgeReport(report, clickTs, expectedContext = {}) {
        ...
        if (isSuccessStatus(status)) {
            return {
                status: 'success',
                message: report.message || 'Codage CIM-10 termine.',
                report: summarizeReport(report)
            };
        }
    ...
    function exposeConsoleFunctions() {
        const api = {
            AUTO_ATCD_BATCH_SCAN: () => scanAndStorePatients(),
            AUTO_ATCD_BATCH_START: () => startBatch(),
            AUTO_ATCD_BATCH_PAUSE: () => pauseBatch(),
            AUTO_ATCD_BATCH_RESUME: () => resumeBatch(),
            AUTO_ATCD_BATCH_RESUME_FROM: (selector, options) => resumeFromPatient(selector, options),
            AUTO_ATCD_BATCH_STOP: () => stopBatch(),
            AUTO_ATCD_BATCH_UNLOCK_STALE: () => clearStaleRunningState('console'),
            AUTO_ATCD_BATCH_DIAG_WEDA_NAV: () => getWedaNavigationDiagnostics(),
            AUTO_ATCD_BATCH_DIAG: (reason) => {
                const diagnostic = buildBlockedBatchDiagnostic(reason || 'console_diagnostic');
                console.log(LOG_PREFIX, 'DIAG', diagnostic);
                return diagnostic;
            },
            AUTO_ATCD_BATCH_STATUS: () => {
                const status = getBatchRuntimeSnapshot();
                console.log(LOG_PREFIX, 'STATUS', status);
                return status;
            },
            AUTO_ATCD_BATCH_LOG: () => showLogs(),
            AUTO_ATCD_BATCH_LOGS: () => getLogs(),
            AUTO_ATCD_BATCH_LOG_CLEAR: () => clearLogs(),
            AUTO_ATCD_BATCH_CLEAR: () => clearBatchData(),
            AUTO_ATCD_BATCH_RETRY_ERRORS: () => retryErrors()
        };
    ...
    function init() {
        ...
        installLocalStorageBridgeListener();
